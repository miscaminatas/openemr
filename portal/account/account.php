...
if ($action == 'verify_email') {
    if (!empty($_SESSION['verifyPortalEmail']) && ($_SESSION['verifyPortalEmail'] === true)) {
        if (!empty($GLOBALS['portal_onsite_two_register']) && !empty($GLOBALS['google_recaptcha_site_key']) && !empty($GLOBALS['google_recaptcha_secret_key'])) {
            ...
            if (!$recaptcha) {
                echo xlt("Algo no salió bien! Ocurrió un error!. Sugerimos contactar a admin@miscaminatas.com.");
                cleanupRegistrationSession();
                exit;
            }
            // process
            ...
            if ($rtn) {
                Header::setupHeader();
                echo '<div class="alert alert-success" role="alert">' . xlt("Revise su casilla de correo [Tal vez su casilla de SPAM]. Recibirá instrucciones para completar el registro. Si no lo ha recibido, por favor contactar a admin@miscaminatas.com.") . '</div>';
            } else {
                echo xlt("Algo salió mal!. Por favor contactar a admin@miscaminatas.com.");
            }
        }
    }
    cleanupRegistrationSession();
    ...
    if (($_SESSION['register_silo_ajax'] ?? null) === true && ($_SESSION['register'] ?? null) === true && isset($_SESSION['pid'])) {
        if (!empty($GLOBALS['portal_onsite_two_register']) && !empty($GLOBALS['google_recaptcha_site_key']) && !empty($GLOBALS['google_recaptcha_secret_key'])) {
            ...
            if ($rtn) {
                ...
                Header::setupHeader();
                echo '<div class="alert alert-success" role="alert">' . xlt("Le enviamos su nueva credencial. Verifique su email inbox [Incluido el SPAM]. Una vez que ingrese podrá chatear con nosotros. Esperamos contactarlo pronto!") . '</div>';
            } else {
                (new SystemLogger())->debug("account.php action do_signup apparently not successful");
                Header::setupHeader();
                echo '<div class="alert alert-danger" role="alert">' . xlt("Problemas con su registro!. Sugerimos contactar a admin@miscaminatas.com para asistencia.") . '</div>';
            }
        } else {

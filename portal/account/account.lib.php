...
function verifyEmail(string $languageChoice, string $fname, string $mname, string $lname, string $dob, string $email): bool
{
    ...
    if ($emailPrepSend) {
        ...
        $mail->Subject = xlt('Verifique su email para registrarse en el portal de los usuarios');
        $mail->MsgHTML($htmlMessage);
        $mail->IsHTML(true);
        $mail->AltBody = $plainMessage;

        if ($mail->Send()) {
            EventAuditLogger::instance()->newEvent('patient-reg-email-verify', '', '', 1, "El email de verificación se envió correctamente a " . $email);
            (new SystemLogger())->debug("El email de verificación se envió correctamente a " . $email);
            return true;
        } else {
            $email_status = $mail->ErrorInfo;
            EventAuditLogger::instance()->newEvent('patient-reg-email-verify', '', '', 0, "El email de verificación NO se envió correctamente a " . $email . " because of following issue: " . $email_status);
            (new SystemLogger())->error("El email de verificación NO se envió correctamente a " . $email . " because of following issue: " . $email_status);
            return false;
        }
    }
